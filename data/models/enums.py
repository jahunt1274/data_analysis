...
class UserType(str, Enum):
    ...
    def from_affiliation(cls, affiliation: Dict) -> 'UserType':
        ...
        if "student_type" in affiliation:
            ...
            if "undergraduate" in student_type.lower():
                return cls.UNDERGRADUATE
            elif "graduate" in student_type.lower():
                return cls.GRADUATE
            elif "mba" in student_type.lower():
                return cls.MBA
            elif "phd" in student_type.lower():
                return cls.PHD
        ...
        if "title" in affiliation:
            ...
            if title:
                if "faculty" in title.lower() or "professor" in title.lower():
                    return cls.FACULTY
...
class DisciplinedEntrepreneurshipStep(str, Enum):
    ...
    in the steps collection when framework is "Disciplined Entrepreneurship".
    """
    ...
    # Class variables for step ordering
    _step_order: ClassVar[List[str]] = [
        "market-segmentation",
        "beachhead-market",
        "end-user-profile",
        "beachhead-tam-size",
    ...
    # FUTURE: Add mapping of common name variations (with spaces, without hyphens, etc.)
    ...
    def get_step_number(cls, step) -> int:
        ...
        try:
            return cls._step_order.index(step_value) + 1
        except ValueError:
            return 0
    
    @classmethod
    def get_by_step_number(cls, step_number: int) -> Optional['DisciplinedEntrepreneurshipStep']:
        """Get the step enum by its number (1-24)"""
        if not isinstance(step_number, int) or step_number < 1 or step_number > len(cls._step_order):
            return None
            
        step_value = cls._step_order[step_number - 1]
        for member in cls:
...
class StepPrefix(str, Enum):
    ...
    SELECTED = "selected-"  # User-selected content (e.g., "selected-market-segmentation")
