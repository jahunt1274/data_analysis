...
class EngagementVisualizer:
    ...
    def create_engagement_metrics_visualization(
        ...
        idea_data = {"New Ideas": {str(i): val for i, val in enumerate(new_ideas)}}
        ...
        if retention_factors:
            ...
            if team_data:
                ...
                plot_bar(
                    axs_flat[3],
                    data={
                        categories[i]: dropout_rates[i] for i in range(len(categories))
                    },
    ...
    def create_demographic_engagement_visualization(
        ...
        gs = plt.GridSpec(
            len(available_demographics), 2, figure=fig, width_ratios=[2, 1]
        )
        ...
        for i, demo_type in enumerate(available_demographics):
            ...
            if categories:
                ...
                filtered_data = {
                    cat: data[cat] for cat in sorted_categories if cat in data
                }
    ...
    def create_burst_activity_visualization(
        ...
        if day_distribution:
            ...
            day_data = {
                day: day_distribution.get(day, 0) * 100 for day in days_order
            }  # Convert to percentage
            ...
            weekend_data = burst_data.get("day_of_week", {}).get(
                "weekend_vs_weekday", {}
            )
            if weekend_data:
        ...
        if length_distribution:
            ...
            length_data = {
                length: length_distribution.get(length, 0)
    ...
    def create_semester_comparison_visualization(
        ...
        if engagement_metrics:
            # Prepare data for grouped bar chart
            ideas_per_user = {
                sem: metrics.get("avg_ideas_per_user", 0)
                for sem, metrics in engagement_metrics.items()
            }
            steps_per_user = {
                sem: metrics.get("avg_steps_per_user", 0)
                for sem, metrics in engagement_metrics.items()
            }
    ...
    def create_tool_version_impact_visualization(
        ...
        user_counts = {
            ver: data.get("user_count", 0) for ver, data in version_comparison.items()
        }
        ...
        if version_comparison:
            # Prepare data for grouped bar chart
            ideas_per_user = {
                ver: data.get("avg_ideas_per_user", 0)
                for ver, data in version_comparison.items()
            }
            steps_per_user = {
                ver: data.get("avg_steps_per_user", 0)
                for ver, data in version_comparison.items()
            }
        ...
        if version_comparison:
            conversion_rates = {
                ver: data.get("idea_to_step_conversion", 0) * 100
                for ver, data in version_comparison.items()
            }
        ...
        if feature_impact or cohort_metrics:
            ...
            for version in versions:
                ...
                high_engagement_pct = (
                    ver_data.get("high_engagement_percentage", 0) * 100
                )
        ...
        if version_comparison and len(versions) >= 2:
            ...
            if latest_ver != "none" and prior_ver != "none":
                ...
                ideas_improvement = (
                    ...
                    / max(0.01, prior_data.get("avg_ideas_per_user", 0))
                    - 1
                ) * 100

                steps_improvement = (
                    latest_data.get("avg_steps_per_user", 0)
                    / max(0.01, prior_data.get("avg_steps_per_user", 0))
                    - 1
                ) * 100

                conversion_improvement = (
                    latest_data.get("idea_to_step_conversion", 0)
                    / max(0.01, prior_data.get("idea_to_step_conversion", 0))
                    - 1
                ) * 100
        ...
        if time_of_day:
            ...
            if hour_distribution:
                ...
                for hour in range(24):
                    hour_str = str(hour)
                    if hour_str in hour_distribution:
                        hour_data[f"{hour:02d}:00"] = (
                            hour_distribution[hour_str] * 100
                        )  # Convert to percentage
                    else:
                        hour_data[f"{hour:02d}:00"] = 0
    ...
    def create_user_timeline_visualization(
        ...
        if events_for_timeline:
            ...
            for date_key in sorted(events_by_date.keys()):
                ...
                # Count by type
                idea_count = sum(
                    1 for e in date_events if e["category"] == "idea_creation"
                )
                step_count = sum(
                    1 for e in date_events if e["category"] == "step_creation"
                )
                other_count = sum(
                    1
                    for e in date_events
                    if e["category"] not in ["idea_creation", "step_creation"]
                )
            ...
            ax_timeline.fill_between(
                date_list, 0, idea_counts, label="Ideas", alpha=0.7, color="#FFC107"
            )
            ax_timeline.fill_between(
                date_list,
                idea_counts,
            ...
            if any(other_counts):
                ax_timeline.fill_between(
                    ...
                    [idea_counts[i] + step_counts[i] for i in range(len(idea_counts))],
                    [
                        idea_counts[i] + step_counts[i] + other_counts[i]
                        for i in range(len(idea_counts))
                    ],
                    label="Other",
                    alpha=0.7,
                    color="#28A745",
            ...
            ax_timeline.set_title(
                f"Activity Timeline for {user_info.get('name', 'User')}"
            )
            ...
            ax_timeline.xaxis.set_major_formatter(
        ...
        idea_events = [
            e for e in activity_timeline if e["event_type"] == "idea_creation"
        ]
        ...
        if idea_events:
            ...
            ax_ideas.pie(
                ...
                labels=(
                    [f"{idea_count} Ideas", ""] if idea_count > 0 else ["No Ideas", ""]
                ),
        ...
        step_events = [
            e for e in activity_timeline if e["event_type"] == "step_creation"
        ]
        ...
        if engagement_summary:
            ...
            status_color = (
                "#28A745" if is_active else "#DC3545"
            )  # Green if active, red if not
    ...
    def create_engagement_report(
        ...
        save_figure(
            fig_demographics, "demographic_analysis", directory=str(figures_path)
        )
        ...
        if include_user_details:
            ...
            engagement_levels = self._analyzer.classify_users_by_engagement(
                course_id=course_id
            )
    ...
    def _create_report_readme(
        self, report_path: Path, course_id: Optional[str] = None
    ) -> None:
        """
...
"""
    ...
    def create_engagement_heatmap(
        ...
        engagement_data = self._analyzer.get_engagement_metrics_over_time(
            interval=(
                time_unit if time_unit != "hour" else "day"
            ),  # 'hour' not supported directly
        )

        # Get additional data for hourly patterns if needed
        usage_patterns = None
        if time_unit == "hour":
            usage_patterns = self._analyzer.get_usage_pattern_analysis(
                course_id=course_id
            )
        ...
        if time_unit == "hour" and usage_patterns:
            ...
            data, row_labels, col_labels = self._prepare_hourly_heatmap_data(
                usage_patterns
            )
    ...
    def _prepare_hourly_heatmap_data(self, usage_patterns):
        ...
        day_distribution = usage_patterns.get("day_of_week", {}).get(
            "day_distribution", {}
        )
        ...
        for hour in range(24):
            ...
            if day_distribution:
                ...
                for i, day in enumerate(days):
                    day_value = day_distribution.get(
                        day, 1 / 7
                    )  # Default to even distribution
                    data[i, hour] = (
                        hour_value * day_value * 7
                    )  # Scale to maintain total activity
            else:
    ...
    def _prepare_daily_heatmap_data(self, engagement_data, metric):
        ...
        return {"values": data, "row_labels": week_labels, "col_labels": day_labels}
    ...
    def _prepare_monthly_heatmap_data(self, engagement_data, metric):
        ...
        return {"values": data, "row_labels": year_labels, "col_labels": month_labels}

    def __str__(self):
        """String representation of EngagementVisualizer."""
        return (
            f"EngagementVisualizer(output_dir={self._output_dir}, theme={self._theme})"
        )
