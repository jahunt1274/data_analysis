...
def compare_framework_versions(
    ...
    """
    Compare framework progression metrics between different tool versions.

    Args:
        version1_data: Data for first version
        version2_data: Data for second version
        framework_steps: List of all steps in the framework

    Returns:
        Dict with version comparison analysis
    """
    result = {
        "step_completion_differences": {},
        "time_differences": {},
        "overall_metrics": {},
    }

    # Compare step completion rates
    v1_completion = version1_data.get("step_completion_rates", {})
    v2_completion = version2_data.get("step_completion_rates", {})

    for step in framework_steps:
        v1_rate = v1_completion.get(step, 0)
        v2_rate = v2_completion.get(step, 0)

        result["step_completion_differences"][step] = {
            "v1_rate": v1_rate,
            "v2_rate": v2_rate,
            "absolute_difference": v2_rate - v1_rate,
            "percentage_change": (
                ((v2_rate - v1_rate) / v1_rate * 100) if v1_rate > 0 else 0
            ),
        }

    # Compare time intervals
    v1_intervals = version1_data.get("interval_statistics", {})
    v2_intervals = version2_data.get("interval_statistics", {})

    if v1_intervals and v2_intervals:
        v1_avg = v1_intervals.get("avg_minutes", 0)
        v2_avg = v2_intervals.get("avg_minutes", 0)

        result["time_differences"] = {
            "v1_avg_interval": v1_avg,
            "v2_avg_interval": v2_avg,
            "absolute_difference": v2_avg - v1_avg,
            "percentage_change": (
                ((v2_avg - v1_avg) / v1_avg * 100) if v1_avg > 0 else 0
            ),
            "time_efficiency_improved": v2_avg < v1_avg,
        }

    # Compare overall metrics
    v1_overall = version1_data.get("overall_metrics", {})
    v2_overall = version2_data.get("overall_metrics", {})

    if v1_overall and v2_overall:
        metrics_to_compare = [
            "avg_steps_per_idea",
        ...
        for metric in metrics_to_compare:
            v1_value = v1_overall.get(metric, 0)
            v2_value = v2_overall.get(metric, 0)

            result["overall_metrics"][metric] = {
                "v1_value": v1_value,
                "v2_value": v2_value,
                "absolute_difference": v2_value - v1_value,
                "percentage_change": (
                    ((v2_value - v1_value) / v1_value * 100) if v1_value > 0 else 0
                ),
            }
    ...
    # Check step completion changes
    improved_steps = sum(
        1
        for step_diff in result["step_completion_differences"].values()
        if step_diff["absolute_difference"] > 0.05
    )
    worsened_steps = sum(
        1
        for step_diff in result["step_completion_differences"].values()
    ...
    if improved_steps > worsened_steps:
        improvements.append("step completion rates")
    elif worsened_steps > improved_steps:
        regressions.append("step completion rates")

    # Check time interval changes
    if result["time_differences"].get("time_efficiency_improved", False):
        improvements.append("time efficiency")
    elif (
        "time_efficiency_improved" in result["time_differences"]
        and not result["time_differences"]["time_efficiency_improved"]
    ):
        regressions.append("time efficiency")

    # Check overall metrics
    metrics_improved = sum(
        1
        for metric_diff in result["overall_metrics"].values()
        if metric_diff["absolute_difference"] > 0
    )
    metrics_worsened = sum(
        1
        for metric_diff in result["overall_metrics"].values()
        if metric_diff["absolute_difference"] < 0
    )

    if metrics_improved > metrics_worsened:
        improvements.append("overall metrics")
    elif metrics_worsened > metrics_improved:
        regressions.append("overall metrics")
